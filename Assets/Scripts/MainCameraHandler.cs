...
public class MainCameraHandler : MonoBehaviour
{
	...
	public static void PlayLoopClip(bool overwrite = false)
	{
		...
		{
			staticAudioSource.clip = ((Game.isLoading && Game.storyState == Game.StoryState.StartStory) || Game.isStory) && (storymusic != 0) ? staticStoryClip[storymusic] : (Game.isPlaying || Game.isLoading) ? staticBackgroundClip[Game.currentLevel] : staticLobbyClip[Game.currentLevel];
